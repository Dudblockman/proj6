...
class ImageLoader(data.Dataset):
  ...
  def load_imagepaths_with_labels(self, class_labels) -> List[Tuple[str, int]]:
    ...
    for label in class_labels.items():
      for file in os.listdir(os.path.join(self.curr_folder, label[0])):
        img_paths.append((os.path.join(self.curr_folder, label[0], file), label[1]))
  ...
  def get_classes(self) -> dict:
    ...
    i = 0
    for directory in os.listdir(self.curr_folder):
      classes[directory] = i
      i+=1
  ...
  def __getitem__(self, index: int) -> Tuple[torch.tensor, int]:
    ...
    pic = self.load_img_from_path(self.dataset[index][0])
    if self.transform:
